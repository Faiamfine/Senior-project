# -*- coding: utf-8 -*-
"""
Created on Wed Mar 23 14:15:38 2022
...
def get_callbacks():
    model_checkpoint = keras.callbacks.ModelCheckpoint(
        os.path.join(r'D:\Faii\Dataset_flood\model', "activelearning-entropy-resnet50v2.h5"),
        monitor='val_loss',
        mode='min',
        save_best_only=True
    )
...
model.save_weights(os.path.join(r'D:\Faii\Dataset_flood\model', "activelearning-entropy-resnet50v2(initial).h5"))
...
for k in range(num_iter):
    ...
    # Reset weights
    model.load_weights(os.path.join(r'D:\Faii\Dataset_flood\model', "activelearning-entropy-resnet50v2(initial).h5"))
    ...
    #     ax[i, 0].set_ylabel(f"Label: {label_u[i]}\nU:{entropy[sorted_indices][i]:.2f}\n{name_u[i]}", rotation=0)
    # fig.savefig(os.path.join(r"D:\Faii\result_each_iter_entropy", f"iter_{k}.png"), bbox_inches="tight")
...
plt.grid()
# plt.savefig(os.path.join(r"D:\Faii\result\Result_final_al", "acc-activelearning-entropy-resnet50v2.png"), bbox_inches="tight")
...
plt.legend(['train', 'test'], loc='upper left')
plt.grid()
# plt.savefig(os.path.join(r"D:\Faii\result\Result_final_al", "loss-activelearning-entropy-resnet50v2.png"), bbox_inches="tight")
#%%
list_val_acc = []
list_val_loss = []
...
# plt.savefig(os.path.join(r"D:\Faii\result\Result_final_al", "each_iter_entropy.png"), bbox_inches="tight")
#%%
import pickle

with open(r'entropy_result_val_acc','wb') as f:
    pickle.dump(list_val_acc,f)
with open(r'entropy_result_val_acc','rb') as f:
    val_acc = pickle.load(f)
print(val_acc)


with open(r'entropy_result_val_loss','wb') as f:
    pickle.dump(list_val_loss,f)
with open(r'entropy_result_val_loss','rb') as f:
