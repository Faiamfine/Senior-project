...
if __name__ == "__main__":
    ...
    prefix_list = ["result_train_acc", "result_train_loss", "result_val_acc", "result_val_loss"]
    prefix_names = {entropy_dir:"en_",
                    ml_dir:"ml_", 
                    u_dir:"lc_"}
    ...
    for val, val_prefix in zip(values_lists, prefix_list):
        ...
        for folder in [entropy_dir, u_dir, ml_dir]:
            ...
            plt.errorbar(x=np.arange(len(means))*50 +100, y=means, yerr= stds, label=prefix_names[folder][:2],markersize=8, capsize=30,fmt='o')
        plt.title(val)
