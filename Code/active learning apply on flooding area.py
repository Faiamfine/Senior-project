...
pool_data_sample=np.load(r"D:\Faii\Dataset_flood\dataset-input-output-for training\X.npy")[:, :, :, :3]
pool_img_label = np.load(r"D:\Faii\Dataset_flood\dataset-input-output-for training\y.npy")
# pool_img_label = to_categorical(pool_img_label, 2, dtype="uint8")
#%%
scaler = StandardScaler()
pool_data_sample = scaler.fit_transform(pool_data_sample.reshape(-1, pool_data_sample.shape[-1])).reshape(pool_data_sample.shape)
#%%
...
model.save_weights(os.path.join(r'D:\Faii\Dataset_flood\model', "activelearning-custom(initial).h5"))
...
for k in range(num_iter):
    ...
    # Reset weights
    model.load_weights(os.path.join(r'D:\Faii\Dataset_flood\model', "activelearning-custom(initial).h5"))
    ...
    # Train
    history = model.fit(
        x=train_data_sample, y=train_img_label,
    ...
    img_u = scaler.inverse_transform(pool_data_sample[adding_index].reshape(-1, pool_data_sample[adding_index].shape[-1])).reshape(pool_data_sample[adding_index].shape)
    ...
    fig.savefig(os.path.join(r"D:\Faii\result_each_iter", f"iter_{k}.png"), bbox_inches="tight")
    
    # Add data
    train_data_sample = np.concatenate((train_data_sample, pool_data_sample[adding_index]), axis=0)
    train_img_label = np.concatenate((train_img_label, pool_img_label[adding_index]), axis=0)
