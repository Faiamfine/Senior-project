...
for file in dirs:
    ...
    arr_im_VV = (arr_im_VV-np.nanmin(arr_im_VV))*(1-0)/(np.nanmax(arr_im_VV)-np.nanmin(arr_im_VV))+0
    arr_im_VH = (arr_im_VH-np.nanmin(arr_im_VH))*(1-0)/(np.nanmax(arr_im_VH)-np.nanmin(arr_im_VH))+0
    ...
    for i in range(arr_im_VV.shape[0]-2):
        for x in range(0,step_size_col):
            for y in range(0,step_size_row): 
                crop_im_VV = arr_im_VV[i:i+3,y*window_size:(y*window_size)+window_size,x*window_size:(x*window_size)+window_size]
                crop_im_VH = arr_im_VH[i:i+3,y*window_size:(y*window_size)+window_size,x*window_size:(x*window_size)+window_size]
                
                crop_im_VV = np.moveaxis(crop_im_VV,0,-1)
                crop_im_VH = np.moveaxis(crop_im_VH,0,-1)
                crop_im_VVVH = crop_im_VV-crop_im_VH
                crop_im_VVVH = (crop_im_VVVH-crop_im_VVVH.min())/(crop_im_VVVH.max()-crop_im_VVVH.min())
                # list_crop_im.append(crop_im)
                #stack
                crop_im = np.concatenate([crop_im_VV,crop_im_VH],axis=-1)
                np.save(os.path.join(r'D:\Faii\Dataset_flood\crop_im',f'{file.split(".")[0]}_{y*window_size}-{(y*window_size)+window_size}_{x*window_size}-{(x*window_size)+window_size}_{i}-{i+3}.png'),crop_im)
                # plt
                plt.close('all')
                ...
                fig.savefig(os.path.join(r'D:\Faii\Dataset_flood\crop_im_for_visualize',f'{file.split(".")[0]}_{y*window_size}-{(y*window_size)+window_size}_{x*window_size}-{(x*window_size)+window_size}_{i}-{i+3}.png'), bbox_inches="tight")
                print((i,i+3),y*window_size,(y*window_size)+window_size,x*window_size,(x*window_size)+window_size)
